...
class ControllerModuleAttributico extends Controller {
    ...
    public function getCategoryAttributes() {
        ...
        foreach ($category_attribute_data as $result) {
            $categoryAttributes[] = $result['attribute_id'];
        }

        $allAttributes = $this->model_catalog_attributico->getAttributes();

        foreach ($allAttributes as $attribute) {
            if (false !== array_search($attribute['attribute_id'], $categoryAttributes)) {
                $json[] = array(
    ...
    public function getAttributeValues() {
        ...
        $method = isset($this->request->get['method']) ? $this->request->get['method'] : 'template';
    ...
    public function getAttributeGroupTree() {
        ...
        if (!$cache_tree_data) {
            ...
            foreach ($attribute_groups as $attribute_group) {
                ...
                $groupNode->addSibling(new Node(array("title" => $attribute_group['name'] . $debug_group, "key" => "group_" . (string) $attribute_group['attribute_group_id'], "folder" => true,
    ...
    private function getAttributeNodes($attribute_group_id, $language_id, $sortOrder, $children, $lazyLoad) {
        ...
        foreach ($attributes as $attribute) {
            $templateNode = new Node(array("title" => $this->session->data['entry_attribute_template'][$language_id], "unselectable" => true, "key" => "template_" . (string) $attribute['attribute_id'],
                "children" => $lazyLoad ? '' : $this->getAttributeValuesNodes($attribute['attribute_id'], $language_id, 'template'), "lazy" => $lazyLoad ? true : false,));
            $valueNode = new Node(array("title" => $this->session->data['entry_attribute_values'][$language_id], "unselectable" => true, "key" => "value_" . (string) $attribute['attribute_id'],
    ...
    public function getCategoryAttributeTree() { // Category_attribute_tree
        $language_id = isset($this->request->get['language_id']) ? $this->request->get['language_id'] : $this->config->get('config_language_id');
        $sortOrder = isset($this->request->get['sortOrder']) ? filter_var($this->request->get['sortOrder'], FILTER_VALIDATE_BOOLEAN) : true;
        ...
        if (is_numeric($category_id) && $category_id !== '0') {
            ...
            foreach ($categoryAttributes as $attribute) {
                ...
                $debug_attribute = $this->debug_mode ? " (id=" . $attribute['attribute_id'] . ")" : '';
                $attributeNode->addSibling(new Node(array("title" => $attribute['attribute_description'] . ' (' . $attribute_group['name'] . ')' . $debug_attribute,
    ...
    public function getProductTree() {
        ...
        if (is_numeric($attribute_id) && $attribute_id !== '0') {
            ...
            foreach ($products as $product) {
                ...
                //  $childNode = new Node();
                $product_item = new Node(array("title" => $product['product_name'] . ' (id=' . $product['product_id'] . ', model=' . $product['model'] . ')' . $debug_category,
