...
function reloadAttribute() {
    if (arguments.length == 0) {
        ...
        var node = arguments[0], self = arguments[1];
        $attribute_synchro_trees.each(function (indx, element) {
            var tree = $("#" + element.id).fancytree("getTree");
...
function addAttribute(activeNode, activeKey, lng_id) {
    var node = activeNode, parentLevel = (activeKey === 'attribute') ? 2 : 1;
    while (node.getLevel() > parentLevel) {
        node = node.getParent();
    }
    $.ajax({
        data: {'user_token': user_token, 'token': token, 'key': node.key, name: activeKey === 'attribute' ? textNewAttribute : textNewGroup},
        url: 'index.php?route=' + extension + 'module/attributico/addAttribute',
        success: function (new_id) {
            node.editCreateNode("child", {title: activeKey === 'attribute' ? textNewAttribute[lng_id] : textNewGroup[lng_id], key: activeKey + "_" + new_id, folder: (activeKey === 'group') ? true : false});
        }
    });
}
...
function deleteAttribute(node) {
    ...
    if (node.getLevel() === 2 || node.getLevel() === 3) {
        ...
        $.ajax({
            data: {'user_token': user_token, 'token': token, 'keys': selNodes ? getSelectedKeys(selNodes) : [node.key]},
...
function copyAttributes(targetNode) {
    ...
    if (node.key.indexOf('group') + 1) {
        $.ajax({
            data: {'user_token': user_token, 'token': token, 'target': node.key, 'attributes': clipboardTitles},
    ...
    if (node.key.indexOf('category') + 1) {
        $.ajax({
            data: {'attributes': selNodes ? getSelectedKeys(selNodes) : [clipboardNodes[0][1].key], 'category_id': node.key, 'categories': selCategories ? getSelectedKeys(selCategories) : [node.key]},
...
function deleteAttributesFromCategory(node, targetnode) {
    ...
    $.ajax({
        data: {'attributes': selNodes ? getSelectedKeys(selNodes) : [node.key], 'category_id': category_id, 'categories': selCategories ? getSelectedKeys(selCategories) : [category_id]},
...
function addAttributeToCategory(targetnode, data, remove) {
    $.ajax({
        data: {'attributes': selNodes ? getSelectedKeys(selNodes) : [data.otherNode.key], 'category_id': targetnode.key, 'categories': selCategories ? getSelectedKeys(selCategories) : [targetnode.key]},
        url: 'index.php?route=' + extension + 'module/attributico/addCategoryAttributes' + '&user_token=' + user_token + '&token=' + token,
        type: 'POST'
    }).done(function () {
        if (!remove) {
...
var clipboardNodes = [], clipboardTitles = [];
var pasteMode = null;
...
function initTrees() {
    $attribute_group_tree.each(function (indx, element) {
        ...
        attribute_group_tree.fancytree({
            ...
            quicksearch: true,
            source: {
                data: {'user_token': user_token, 'token': token, 'language_id': lng_id, 'sortOrder': sortOrder, 'lazyLoad': lazyLoad, 'tree': "1", 'isPending': false},
            ...
            lazyLoad: function (event, data) {
                data.result = {
                    data: {'user_token': user_token, 'token': token, 'key': data.node.key, 'language_id': lng_id}, // cache:true,
                    url: 'index.php?route=' + extension + 'module/attributico/getLazyAttributeValues'
                };
            },
            edit: {
                triggerStart: ["f2", "shift+click", "mac+enter"],
                inputCss: {minWidth: "18em"},
                ...
                save: function (event, data) {
                    ...
                    $.ajax({
                        data: {'user_token': user_token, 'token': token, 'key': data.node.key, 'name': data.input.val(), 'language_id': lng_id, 'oldname': data.orgTitle},
            ...
            dnd: {
                ...
                dragDrop: function (targetNode, data) {
                    ...
                    $.ajax({
                        data: {'user_token': user_token, 'token': token,
        ...
        $("input[name=" + currentTab + "_search" + lng_id + "]").keyup({tree: currentTree, tab: currentTab, lng_id: lng_id}, FilterSearch).focus();
        $("button#" + currentTab + "_btnSearch" + lng_id).click({tree: currentTree, tab: currentTab, lng_id: lng_id}, FilterButtonSearch);
        $("div#" + currentTab + "_filter" + lng_id + " input:checkbox").change({tree: currentTree, tab: currentTab, lng_id: lng_id}, FilterSettingsChange);
...
// --------------------------------------- category tree ------------------------------------------------
    $category_tree.each(function (indx, element) {
        ...
        category_tree.fancytree({
            ...
            source: {
                data: {'user_token': user_token, 'token': token, 'language_id': lng_id, 'sortOrder': sortOrder},
            ...
            activate: function (event, data) {
                ...
                tree.reload({
                    data: {'user_token': user_token, 'token': token, 'language_id': lng_id, 'category_id': currentCategory, 'sortOrder': $('input[id = "sortOrder_category_attribute_tree' + lng_id + '"]:checkbox').is(":checked"), 'tree': "4"},
...
// --------------------------------------- category attribute tree -------------------------------------
    $category_attribute_tree.each(function (indx, element) {
        ...
        category_attribute_tree.fancytree({
            ...
            extensions: ["edit", "dnd"],
            source: {
                data: {'user_token': user_token, 'token': token, 'language_id': lng_id, 'category_id': currentCategory, 'sortOrder': sortOrder, 'tree': "4"},
                url: 'index.php?route=' + extension + 'module/attributico/getCategoryAttributeTree'
            },
            loadError: function (e, data) {
                var error = data.error;
            ...
            edit: {
                ...
                edit: function (event, data) {
                    data.input.dropmenu({
                        'source': function (request, response) {
                            $.ajax({
                                data: {'user_token': user_token, 'token': token, 'filter_name': encodeURIComponent(request), 'language_id': lng_id},
                ...
                save: function (event, data) {
                    $.ajax({
                        data: {'attributes': [data.node.key], 'category_id': data.node.getParent().key, 'categories': selCategories ? getSelectedKeys(selCategories) : [data.node.getParent().key]},
                        url: 'index.php?route=' + extension + 'module/attributico/addCategoryAttributes' + '&user_token=' + user_token + '&token=' + token,
                        type: 'POST'
                    }).done(function () {
                        $(data.node.span).removeClass("pending");
    ...
    $attribute_tree.each(function (indx, element) {
        ...
        attribute_tree.fancytree({
            ...
            quicksearch: true,
            source: {
                data: {'user_token': user_token, 'token': token, 'language_id': lng_id, 'sortOrder': sortOrder, 'lazyLoad': lazyLoad, 'isPending': false, 'tree': "3"},
                url: 'index.php?route=' + extension + 'module/attributico/getAttributeGroupTree'
            },
            loadError: function (e, data) {
                var error = data.error;
    ...
    $duty_attribute_tree.each(function (indx, element) {
        ...
        duty_attribute_tree.fancytree({
            ...
            quicksearch: true,
            source: {
                data: {'user_token': user_token, 'token': token, 'language_id': lng_id, 'sortOrder': sortOrder, 'lazyLoad': lazyLoad, 'isPending': false, 'tree': "2"},
                url: 'index.php?route=' + extension + 'module/attributico/getAttributeGroupTree'
            },
            loadError: function (e, data) {
                var error = data.error;
    ...
    $attribute_product_tree.each(function (indx, element) {
        ...
        attribute_product_tree.fancytree({
            ...
            quicksearch: true,
            source: {
                data: {'user_token': user_token, 'token': token, 'language_id': lng_id, 'sortOrder': sortOrder, 'lazyLoad': lazyLoad, 'isPending': false, 'tree': "5"},
                url: 'index.php?route=' + extension + 'module/attributico/getAttributeGroupTree'
            },
            loadError: function (e, data) {
                var error = data.error;
            ...
            activate: function (event, data) {
                ...
                tree.reload({
                    data: {'user_token': user_token, 'token': token, 'language_id': lng_id, 'attribute_id': currentAttributeID, 'title': data.node.title, 'sortOrder': sortOrder},
    ...
    $product_tree.each(function (indx, element) {
        ...
        product_tree.fancytree({
            ...
            minExpandLevel: 2,
            source: {
                data: {'user_token': user_token, 'token': token, 'language_id': lng_id, 'attribute_id': currentAttributeID, 'sortOrder': sortOrder},
                url: 'index.php?route=' + extension + 'module/attributico/getProductTree'
            },
            loadError: function (e, data) {
                var error = data.error;
...
$(function () {     // document ready actions
    ...
    $('[id ^= "tree"]').each(function (indx, element) {
        $(element).fancytree({
            ...
            selectMode: 2,
            source: {
                data: {'user_token': user_token, 'token': token, 'tree': indx + 1},
    ...
    $('input[id ^= "lazyLoad"]:checkbox').change(function (e) {   // on/off lazyload
        var id = $(this).attr("id"), tree = $("#" + id.replace("lazyLoad_", "")).fancytree("getTree"),
    ...
    $('input[id ^= "autoCollapse"]:checkbox').change(function (e) {
        var id = $(this).attr("id"), flag = $(this).is(":checked");
    ...
    $group_check_tree.each(function (indx, element) {
        ...
        $(element).fancytree({
            ...
            autoScroll: true,
            source: {
                data: {'user_token': user_token, 'token': token, 'sortOrder': sortOrder, 'onlyGroup': true, 'isPending': false},
                url: 'index.php?route=' + extension + 'module/attributico/getAttributeGroupTree'
            }
        });
    });
    ...
    $category_check_tree.each(function (indx, element) {
        ...
        $(element).fancytree({
            ...
            source: {
                data: {'user_token': user_token, 'token': token, 'sortOrder': sortOrder},
                url: 'index.php?route=' + extension + 'module/attributico/getCategoryTree'
            }
        });
    });
...
function FilterAction(e, lng_id, tab) {
    $("span#" + tab + "_searchmode" + lng_id + " input#" + tab + "_regex" + lng_id + ":checkbox").prop({"checked": true});
    switch ($(e).attr("id")) {
        case 'f_' + tab + '_empty':
        ...
        case 'f_' + tab + '_default':
            $("span#" + tab + "_searchmode" + lng_id + " input#" + tab + "_regex" + lng_id + ":checkbox").prop({"checked": false});
...
function tools(task) {
    $('[id *= "tree"].options').each(function (indx, element) {
        $(element).fancytree("getTree").generateFormElements(true, true, {filter: null, stopOnParents: false});
    });
    //var complete_img = '<img class= "complete-img" src="view/javascript/fancytree/skin-custom/accept.png"/>  ';
    // alert("POST data:\n" + jQuery.param($("#form-attributico").serializeArray()));
    $.ajax({
        data: {'user_token': user_token, 'token': token, 'task': task, 'options': $("#form-attributico").serialize()},
