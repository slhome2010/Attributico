...
function reloadAttribute(){if(0==arguments.length)$attribute_synchro_trees.each(function(a,d){var c=$("#"+d.id).fancytree("getTree");c.options.source.data.cache=$('input[name = "attributico_cache"]:checkbox').is(":checked");ClearFilter(c);c.reload().done(function(){c.options.source.data.isPending=!1})});else{var a=arguments[0],c=arguments[1];$attribute_synchro_trees.each(function(b,d){var f=$("#"+d.id).fancytree("getTree");f.options.source.data.cache=$('input[name = "attributico_cache"]:checkbox').is(":checked");
if(f!==a.tree||c)ClearFilter(f),f.reload().done(function(){f.options.source.data.isPending=!1;var b=f.getNodeByKey(a.key);b&&void 0!==b&&b.setActive()})});c||a.setActive()}}function ClearFilter(a,c,b){a.isFilterActive()&&(a.clearFilter(),3===arguments.length?($("input[name *= "+c+"_search"+b+"]").val(""),$("span[id *= "+c+"_matches"+b+"]").text("")):($('input[name *= "search"]').val(""),$('span[id *= "matches"]').text("")),$("[id ^=loadImg]").hide())}
...
b){$(b).prop({disabled:!flag})})});$('input[name = "attributico_smart_scroll"]:checkbox').change(function(a){$(this).is(":checked")?$('[id *= "tree"]:not(.settings) > ul.fancytree-container').addClass("smart-scroll"):$("ul.fancytree-container").removeClass("smart-scroll")});$('input[name = "attributico_cache"]:checkbox').change(function(a){$.ajax({data:{user_token:user_token,token:token},url:"index.php?route="+extension+"module/attributico/cacheDelete",success:function(a){$category_synchro_trees.each(function(a,
c){var f=$("#"+c.id).fancytree("getTree");f.options.source.data.cache=$('input[name = "attributico_cache"]:checkbox').is(":checked");f.reload().done(function(){f.options.source.data.isPending=!1;reloadAttribute();reactivateCategory()})})}})});$('input[name = "attributico_multistore"]:checkbox').change(function(a){$.ajax({data:{user_token:user_token,token:token},url:"index.php?route="+extension+"module/attributico/cacheDelete",success:function(a){$category_synchro_trees.each(function(a,c){var f=$("#"+
c.id).fancytree("getTree");f.options.source.data.multistore=$('input[name = "attributico_multistore"]:checkbox').is(":checked");f.reload().done(function(){f.options.source.data.isPending=!1;reloadAttribute();reactivateCategory()})})}})});$('input[id ^= "lazyLoad"]:checkbox').change(function(a){a=$(this).attr("id");a=$("#"+a.replace("lazyLoad_","")).fancytree("getTree");var c=$(this).is(":checked");a.options.source.data.lazyLoad=c;a.reload()});$('input[id ^= "sortOrder"]:checkbox').change(function(a){a=
$(this).attr("id");a=$("#"+a.replace("sortOrder_","")).fancytree("getTree");var c=$(this).is(":checked");a.options.source.data.sortOrder=c;a.options.source.data.category_id=currentCategory;a.reload()});$('input[id ^= "autoCollapse"]:checkbox').change(function(a){a=$(this).attr("id");var c=$(this).is(":checked");$("#"+a.replace("autoCollapse_","")).fancytree("getTree").options.autoCollapse=c});$('input[id ^= "multiSelect"]:checkbox').change(function(a){a=$(this).attr("id");$("#"+a.replace("multiSelect_",
"")).fancytree("getTree").options.selectMode=$(this).is(":checked")?3:2});$group_check_tree.each(function(a,c){var b=$('input[name = "attributico_sortorder"]:checkbox').is(":checked");$(c).fancytree({checkbox:!0,selectMode:3,autoScroll:!0,source:{data:{user_token:user_token,token:token,sortOrder:b,onlyGroup:!0,isPending:!1},url:"index.php?route="+extension+"module/attributico/getAttributeGroupTree"}})});$category_check_tree.each(function(a,c){var b=$('input[name = "attributico_sortorder"]:checkbox').is(":checked");
$(c).fancytree({autoCollapse:!0,autoScroll:!0,minExpandLevel:2,checkbox:!0,selectMode:$('input[id = "input-attributico_multiselect"]:checkbox').is(":checked")?3:2,source:{data:{user_token:user_token,token:token,sortOrder:b},url:"index.php?route="+extension+"module/attributico/getCategoryTree"}})})});
